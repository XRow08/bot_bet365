...
mongoose.connect('mongodb://localhost:27017/strategies', {
...
class StageMachine {
    ...
    constructor(myBrowser, actualStage = Stage.INIT_STAGE) {
        ...
        this.db.run("CREATE TABLE IF NOT EXISTS Strategies (id integer primary key autoincrement, roleta varchar(256), contagens INTEGER, estrategia varchar(256), criadoem datetime);")
     ...
     */
    async init(instance) {
        await instance.myBrowser.pupPage.goto("https://casino.bet365.com/Play/LiveRoulette")
    ...
    async login(instance) {
        ...
        if (element !== null) {
            await instance.myBrowser.pupPage.evaluate(() => {
                ...
                document.querySelector(".modal__button.login-modal-component__login-button").click()
            })
        }
    ...
    async resolveIndentity(instance) {
        await instance.myBrowser.pupPage.evaluate(() => {
            let frames = Array.from(document.querySelectorAll("iframe")).filter(it => it.src.indexOf('https://members.bet365.com/members/services/notifications/process') !== -1)
            if (frames.length !== 0) {
                // Frame existe mas pode é bom checar
                let el = frames[0].contentWindow.document.querySelector("#verifyIdentity")

                if (el !== null) {
                    // botão de verificar existe
                    frames[0].contentWindow.document.querySelector("#remindLater").click()
                }
    ...
    async resolveLastLogin(instance) {
        await instance.myBrowser.pupPage.evaluate(() => {
            ...
            if (popup !== null) {
                ...
                let frames = Array.from(document.querySelectorAll("iframe")).filter(it => it.src.indexOf('https://dl-com.c365play.com/live_desktop/bundles/') !== -1)
    ...
    async openHallPopup(instance) {
        await instance.myBrowser.pupPage.goto("https://dl-com.c365play.com/casinoclient.html?game=rol&preferedmode=real&language=pt&cashierdomain=www.sgla365.com&ngm=1&wmode=opaque")
        return Stage.nextStage(instance.actualStage)
    }
    ...
    async getData(instance) {
        if ((await instance.myBrowser.pupPage.evaluate(() => {
                return document.querySelector(".modal-container.modal-confirm_desktop.modal-confirm_desktop_with-icon")
            })) !== null) {
            return Stage.nextStage(instance.actualStage)
        }
        console.log('exec')
        let result = await instance.myBrowser.pupPage.evaluate(() => {
            ...
            roulettes = Array.from(document.getElementsByClassName('lobby-table__container')), jsonDict = {}, roulettes.forEach((e => {
                // noinspection JSUnresolvedVariable
                if (rouletteName = e.getElementsByClassName('lobby-table__name-container')[0].textContent, 'Spread Bet Roulette' === rouletteName) return;
                let t = [];
                e.querySelectorAll('div[class*=roulette-history-item__value] > div[class*=roulette-history-item__value]').forEach((e => {
        ...
        Object.keys(result).forEach(item => {
            ...
            if (ret !== null) {
                ...
                strategyDocument.save().then(res => {
                ...
                strategyModel.find({}).then(res => {
                        res.forEach(e => {
                            e.strategy.forEach(g => {
                                ...
                                ['Baixo', 'Alto', 'Vermelho', 'Preto', 'Impar', 'Par'].forEach(item2 => {
    ...
    async resolveTimeout(instance) {
        ...
        for (let i = 0; i < 10; i++) {
            let response = await instance.myBrowser.pupPage.evaluate(() => {
                ...
                let frame1 = document.querySelector("iframe[class='inline-games-page-component__game-frame '] ")
                if (frame1 === null) {
                    return false
                }
                let frames = Array.of(frame1.contentDocument.querySelector("#gamecontent"))

                if (frames.length !== 0) {
                    return frames[0].contentWindow.document.querySelectorAll("li[data-automation-locator='button.lobby']").length > 0
                } else {
                    return false
                }
            })
    ...
    saveInDb(data, rouletteName) {
        this.db.run(`INSERT INTO Strategies(roleta, estrategia, contagens, criadoem) VALUES ('${rouletteName}', '${data.name}', ${data.count}, ${new Date().getTime()})`)
        ...
        this.db.parallelize(() => {
            ...
            this.db.parallelize(() => {
                ...
                //   function roleta() {
                this.db.run('ALTER TABLE Strategies RENAME COLUMN roulette TO roleta;', (err, data) => {
                    console.log(data)
                })
                // db.close()
                //   }
                ...
                //   function count() {
                this.db.run('ALTER TABLE Strategies RENAME COLUMN countt TO contagens;', (err, data) => {
                    console.log(data)
                })
                // db.close()
                //   }
                ...
                //   function strategy() {
                this.db.run('ALTER TABLE Strategies RENAME COLUMN strategy TO estrategia;', (err, data) => {
                    console.log(data)
                })
                // db.close()
                //   }
                ...
                this.db.run('ALTER TABLE Strategies RENAME COLUMN created TO criadoem;', (err, data) => {
                ...
                this.db.run('ALTER TABLE Strategies RENAME COLUMN created TO criadoem;', (err, data) => {
                    ...
                    this.db.run("UPDATE Strategies SET data = date(data/1000,'unixepoch');")
                    this.db.run("UPDATE Strategies SET hora = time(hora/1000,'unixepoch');")
                    //    db.close()
                })
                //   }
