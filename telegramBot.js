  ...
  if (fs.existsSync("csv/Strategies.csv")) {
    csvToJSON()
      ...
      .then((obj) => {
       ...
       ⌚️<b>Data</b>: <i>${obj[j].data}</i>\n
       ⌛️<b>Hora</b>: <i>${obj[j].hora}</i> \n
       `,
            {
              parse_mode: "HTML",
            }
...
bot.action("login", async (ctx) => {
  (async () => {
    ...
    await page.waitForFrame(
      "https://members.bet365.com/members/services/notifications/process?em=1&prdid=11",
      {
        visible: true,
      }
    );
    ...
    await page.$(
      'iframe[src="https://members.bet365.com/members/services/notifications/process?em=1&prdid=11"]'
    );
    await page.goto(
      "https://members.bet365.com/members/services/notifications/process?em=1&prdid=11"
    );
    ...
    const elementss = await page.evaluateHandle(() => {
      const allOccurances = [
        ...document.querySelectorAll(".lobby-table__name-container"),
      ];
    ...
    const elements = await page.evaluateHandle(() => {
      const allOccurances = [
        ...document.querySelectorAll(".lobby-tables__item"),
      ];
      const data = allOccurances.map((node) => node.innerHTML);
    ...
    const stfirst12mid = await page.$$(
      ".roulette-table-cell_side-second-dozen"
    );
    const stfirst12last = await page.$$(
      ".roulette-table-cell_side-third-dozen"
    );
    ...
    const lastCollumn = await page.$$(
      ".roulette-table-cell_side-bottom-column"
    );
